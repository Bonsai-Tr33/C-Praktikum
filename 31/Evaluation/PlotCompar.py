...
Data35 = np.array([[x*10**(5),y*10**(-6)] for x,y in zip(Data35Tab['Druck [10^5 Pa]'].tolist(), Data35Tab['Volumen [ml]'].tolist())]) # Pa, m^3

def p_vdw(V):
    return (n*R*T)/(V - n*b) - (a*n**2)/(V**2)

# Plot
Vplot = np.linspace(0, 4*10**(-6), 1000)
