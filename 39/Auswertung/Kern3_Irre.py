...
# ---------
# First measurements
messreihe3 = Messreihe(data_path / 'Ringkern3Irre_39.csv', N1=1010, N2=53, R=0.2, F=1)
Data3 = messreihe3.dataArray()

# shift correction
B_Shift = symmetric(Data3.iloc[:, 1], Data3.iloc[:,2], symPX=6000)
DataH = Data3.iloc[:, 1]*10**(-3) 
DataB = Data3.iloc[:,2] - B_Shift
print('Symmetrisierungsfaktor Ringkern 1: ' + str(B_Shift) + 'T')

# koerzitivfeldstärke measurement 3
HC3, DHC3 = HC(DataH, DataB, B_Tolerance=0.008)
print('Koerzitivfeldstärke Ringkern 3: (' + str(HC3) + ' ± ' + str(DHC3) + ') A/m')

# remanenz measurement 3
BR3, DBR3 = Br(DataH, DataB, H_Tolerance=0.009)
...
plt3in.annotate('',
    xy=(1300*10**(-3), 1.05),     # Pfeilspitze (H, B)
    xytext=(1450*10**(-3), 1.09), # Pfeilanfang (H, B)
    arrowprops=dict(
        arrowstyle='->',
        color='red',
        lw=2
...
plt3in.annotate(
    '',
    xy=(1533*10**(-3), 0.9987),     # Pfeilspitze (H, B)
    xytext=(1303*10**(-3), 0.9778), # Pfeilanfang (H, B)
    arrowprops=dict(
        arrowstyle='->',
        color='red',
        lw=2
...
plt3in.annotate(
    '',
    xy=(1633*10**(-3), 1.0567),     # Pfeilspitze (H, B)
    xytext=(1774*10**(-3), 1.0662), # Pfeilanfang (H, B)
    arrowprops=dict(
        arrowstyle='->',
        color='blue',
        lw=2
...
plt3in.annotate(
    '',
    xy=(994.5*10**(-3), 0.9641),     # Pfeilspitze (H, B)
    xytext=(1083*10**(-3), 0.9948), # Pfeilanfang (H, B)
    arrowprops=dict(
        arrowstyle='->',
        color='blue',
        lw=2
...
plt3in.set_xlim(900*10**(-3),1900*10**(-3))
plt3in.set_ylim(0.95, 1.1)
mark_inset(plt3, plt3in, loc1=1, loc2=2, lw=1)
plt.savefig(img_path / 'Hyster3_irre.png')
plt.show()
