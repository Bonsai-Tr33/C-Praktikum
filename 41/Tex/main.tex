...
\vspace{1em}
	...
	von Signalen gemessen und auf $v_{koaxial} = (192 \pm 18) \cdot 10^6 \, \frac{m}{s}$ und $v_{verzoegert} = (459 \pm 12) \cdot 10^3 \, \frac{m}{s}$ bestimmt. An dem RG213/U Koaxialkabel wurde die Ausbreitungsgeschwindigkeit auch mittels stehender Wellen bestimmt. Bei der Methode der stehenden Wellen lässt sich die Dispersion der 
...
\section{Einleitung}
	...
	Durch den Aufbau des Kabels mit Innenleiter, Dielektrikum und Außenleiter {gibt es in Ausbreitungsrichtung kein elektromagnetisches Feld.}
	Dabei auftretende Dämpfung, Dispersion und Reflexion sind abhängig von Materialparametern, Verlusten im Kabel und von Impedanzanpassungen. \\
	Dieser Versuch beschäftigt sich damit, diese Eigenschaften von Koaxialkabeln genauer zu beleuchten, deren Hintergrund zu verstehen und auch den Umgang mit Koaxialkabeln
	zu erlernen.\\
...
\section{Theorie}
	...
	\subsection{Wellenausbreitung auf Kabeln}
		...
		\subsubsection{Wellenwiderstand}
			...
			Die Induktivität $L^{\ast} = \frac{\text{Induktivität}}{\text{Längeneinheit}}$
			und die Kapazität $C^{\ast} = \frac{\text{Kapazität}}{\text{Längeneinheit}}$. Über
		...
		\subsubsection{Mathematische Beschreibung einer Welle} \label{MathWell}
			...
			Im Fall eines Koaxialkabels kommt es jedoch zu Dämpfung, welche die Amplitude der einlaufenden und der rücklaufenden Welle verschiebt.
...
\section{Experiment und Auswertung}
	...
	\subsection{Signalübertragung auf Koaxialkabel bei verschiedenartiger Anpassung}
		...
		wodurch eine Reflexion mit $\rho = -\frac{1}{3}$ entsteht. Die Amplitude wird reduziert (vgl. Abb. \ref{Kombi50}).
		...
		Die jeweiligen effektiven Widerstände $R_{a,eff}$ wurden mittels der Parallelschaltung von der Leitungsimpedanz und den Zusatzwiderständen berechnet. \\
	...
	\subsection{Messung der Ausbreitungsgeschwindigkeit mithilfe von stehenden Wellen}
		...
		CH I und CH II des Oszilloskops angeschlossen. An dem Generator kann man die Amplitude und die Frequenz des ausgehenden Signals einstellen. Die Mathematische
		...
		Um die Ausbreitungsgeschwindigkeit einer Welle in deinem Koaxialkabel zu messen, wurde zunächst eine stehende, sinusförmige Welle erzeugt.
		...
		\begin{table}[H]          %so funktionieren die Tabellen in LaTeX
			...
			\hline
			$\frac{\lambda}{4}$ 				&     ($938,0 \pm 1,9$)			&     $(187,6 \pm {9,4})$ 				\\

			$\frac{\lambda}{2}$ 				&     ($1892,0 \pm 5,2$)	 	&     $(189,2 \pm {9,5})$ 				\\

			$\frac{3 \cdot \lambda}{4}$ 		&     ($2875,0 \pm 1,9$) 		&     $(191,7 \pm {9,6})$ 				\\
		...
		\end{table}
		\noindent Für die relative Permittivität ergibt sich:
		\begin{table}[H]          %so funktionieren die Tabellen in LaTeX
			\centering
			\begin{tabular}{c|c}
			...
			\hline
			$\frac{\lambda}{4}$ & $(2,55 \pm {0,26})$ \\

			$\frac{\lambda}{2}$ & $(2,51 \pm {0,25})$ \\

			$\frac{3 \cdot \lambda}{4}$ & $(2,5 \pm {0,24})$ \\
			\end{tabular}  
			\caption{relative Permittivität in Wellenlängenabhängigkeit} 			   %siehe Graphik: Beschriftung
		...
		nicht gut einstellen daher wurde ein großzügiger Fehler von 5\% für die Frequenzbestimmung gewählt. \\
		...
		\subsubsection{Bestimmung der Dämpfung des Signals im Koaxialkabel}
			...
			{Das entspricht nicht der physikalischen Erwartung. Dieser Fehltrend in unseren Messungen entsteht durch die sehr ungenaue Bestimmung der Knotenpunkte für die Messungen, da wir die Punkte nicht empirisch, 
	...
	sich selbst abgebildet wird, {das bedeutet, dass der Strahlengang
	...
	dass die LED bei der Startposition $s_{LED} = (23,2 \pm {0,1}) \, cm$ steht.
	Daraus lässt sich die Lichtgeschwindigkeit berechnen:
	\begin{equation*}
		c = \frac{{2 \cdot}s}{\Delta t}
	\end{equation*}
	...
	{Die Ungenauigkeiten rühren daher, dass die Wahl der Startposition der Flanken zur Zeitmessung nur sehr ungenau wählbar ist.
