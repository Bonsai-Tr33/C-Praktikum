...
\vspace{1em}
	...
	von Signalen gemessen \textcolor{red}{und auf $v_{koaxial} = (192 \pm 18) \cdot 10^6 \, \frac{m}{s}$ und $v_{verzoegert} = (459 \pm 12) \cdot 10^3 \, \frac{m}{s}$ bestimmt.} An dem RG213/U Koaxialkabel \textcolor{red}{wurde die Ausbreitungsgeschwindigkeit} auch mittels stehender Wellen \textcolor{red}{bestimmt}. Bei der Methode der stehenden Wellen lässt sich die Dispersion der 
...
\section{Einleitung}
	...
	Durch den Aufbau des Kabels mit Innenleiter, Dielektrikum und Außenleiter \textcolor{red}{gibt es in Ausbreitungsrichtung kein elektromagnetisches Feld.}
	\textcolor{red}{Dabei auftretende Dämpfung, Dispersion und Reflexion sind abhängig von Materialparametern, Verlusten im Kabel und von Impedanzanpassungen.} \\
	\textcolor{red}{Dieser Versuch} beschäftigt sich damit, diese Eigenschaften von Koaxialkabeln genauer zu beleuchten, deren Hintergrund zu verstehen und auch den Umgang mit Koaxialkabeln
	zu erlernen.\\
...
\section{Theorie}
	...
	\subsection{Wellenausbreitung auf Kabeln}
		...
		\subsubsection{Wellenwiderstand}
			...
			\textcolor{red}{Die Induktivität}   $L^{\ast} = \frac{\text{Induktivität}}{\text{Längeneinheit}}$
			und \textcolor{red}{die Kapazität} $C^{\ast} = \frac{\text{Kapazität}}{\text{Längeneinheit}}$. Über
		...
		\subsubsection{Mathematische Beschreibung einer Welle} \label{MathWell}
			...
			Im Fall eines Koaxialkabels kommt es jedoch zu Dämpfung, \textcolor{red}{welche die Amplitude der einlaufenden und der rücklaufenden Welle verschiebt.
			Dadurch nimmt die Amplitude an den Knotenpunkten endliche Werte ungleich Null an.} 
...
\section{Experiment und Auswertung}
	...
	\subsection{Signalübertragung auf Koaxialkabel bei verschiedenartiger Anpassung}
		...
		wodurch eine Reflexion mit \textcolor{red}{$\rho = -frac{1}{3}$} entsteht. Die Amplitude wird reduziert (vgl. Abb. \ref{Kombi50}).
		...
		\textcolor{red}{Die jeweiligen effektiven Widerstände $R_{a,eff}$ wurden mittels der Parallelschaltung von der Leitungsimpedanz und den Zusatzwiderständen berechnet.} \\
	...
	\subsection{Messung der Ausbreitungsgeschwindigkeit mithilfe von stehenden Wellen}
		...
		CH I und CH II des Oszilloskops angeschlossen. An dem Generator kann man die Amplitude und die \textcolor{red}{Frequenz} des ausgehenden Signals einstellen. Die Mathematische
		...
		Um die Ausbreitungsgeschwindigkeit einer Welle in deinem Koaxialkabel zu messen, \textcolor{red}{wurde} zunächst eine stehende, sinusförmige Welle erzeugt.
		...
		\begin{table}[H]          %so funktionieren die Tabellen in LaTeX
			...
			\hline
			$\frac{\lambda}{4}$ 				&     ($938,0 \pm 1,9$)			&     $(187,6 \pm \textcolor{red}{9,4})$ 				\\

			$\frac{\lambda}{2}$ 				&     ($1892,0 \pm 5,2$)	 	&     $(189,2 \pm \textcolor{red}{9,5})$ 				\\

			$\frac{3 \cdot \lambda}{4}$ 		&     ($2875,0 \pm 1,9$) 		&     $(191,7 \pm \textcolor{red}{9,6})$ 				\\
		...
		\end{table}
		\noindent Für die \textcolor{red}{relative Permittivität} ergibt sich:
		\begin{table}[H]          %so funktionieren die Tabellen in LaTeX
			\centering
			\begin{tabular}{c|c}
			...
			\hline
			$\frac{\lambda}{4}$ & $(2,55 \pm \textcolor{red}{0,26})$ \\

			$\frac{\lambda}{2}$ & $(2,51 \pm \textcolor{red}{0,25})$ \\

			$\frac{3 \cdot \lambda}{4}$ & $(2,5 \pm \textcolor{red}{0,24})$ \\
			\end{tabular}  
			\caption{\textcolor{red}{relative Permittivität} in Wellenlängenabhängigkeit} 			   %siehe Graphik: Beschriftung
		...
		nicht gut einstellen \textcolor{red}{daher wurde ein großzügiger Fehler von 5\% für die Frequenzbestimmung gewählt}. \\
		...
		\subsubsection{Bestimmung der Dämpfung des Signals im Koaxialkabel}
			...
			Um die verschiedenen Werte der Dämpfung sinvoll zu veranschaulichen werden sie auf die Kabellänge normiert. Das Ergebnis wird also nicht in $dB$ sondern $dB/m$ angegeben.
			\begin{table}[htbp]          %so funktionieren die Tabellen in LaTeX
				\centering
				\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}c|c|c}
				$\lambda$ (in $m$) 					& $U(x)$ (in $10^{-3} \, V$) & $D$ (in $dB/m$)\\
				\hline
				$\frac{\lambda}{4}$ 				& $(14,20 \pm 0,45)$ & $(0,000 \pm 0,025)$\\
				$\frac{\lambda}{2}$ 				& $(21,8 \pm 1,1)$ & $(-0,075 \pm 0,016)$\\
				$\frac{3 \cdot \lambda}{4}$ 		& $(26,4 \pm 1,1)$ & $(-0,108 \pm 0,013)$\\
				\end{tabular*}  
			...
			\begin{figure}[H]
				...
				\label{Img:D-F}
			\end{figure}
			\noindent In Abbildung~\ref{Img:D-F} ist deutlich zu erkennen, dass das Dämpfungsmaß von der Frequenz abhängt und mit steigender Frequenz sinkt.
			\textcolor{red}{Das entspricht nicht der physikalischen Erwartung. Dieser Fehltrend in unseren Messungen entsteht durch die sehr ungenaue Bestimmung der Knotenpunkte für die Messungen, da wir die Punkte nicht empirisch, 
			sondern nach eigenem Ermessen im Oszilloskop bestimmt haben.}
	...
	sich selbst abgebildet wird, \textcolor{red}{das bedeutet, dass der Strahlengang
	der LED die Strecke s geht, am Ende reflektiert wird und auf den Ursprung (die LED) zurückfällt.}
	Der Strahlteiler $S$ reflektiert das von $T_1$ 
	...
	dass die LED bei der Startposition $s_{LED} = (23,2 \pm \textcolor{red}{0,1}) \, cm$ steht.
	Daraus lässt sich die Lichtgeschwindigkeit berechnen:
	\begin{equation*}
		c = \frac{\textcolor{red}{2 \cdot}s}{\Delta t}
	\end{equation*}

	\begin{figure}
	\centering
		\includegraphics[width=0.7\linewidth]{../Images/SOPLight.png}
	\caption{Distanzen und gemessene Laufzeiten für die Lichtgeschwindigkeitsmessung}
	\label{LichtgeschwindigkeitMessung}
	\end{figure}
	...
	\textcolor{red}{Die Ungenauigkeiten rühren daher, dass die Wahl der Startposition der Flanken zur Zeitmessung nur sehr ungenau wählbar ist.
	Zudem überlappen sich die Signale der Photodiode, was die genaue Bestimmung der Zeitdifferenz erschwert.
	Des Weiteren ist die Ablesegenauigkeit des Oszilloskops begrenzt.}
